...
export default function NewsCard({ item, isLiked, onLike, onDislike, onEnrich, enriching }) {
  ...
  const handleLike = useCallback(() => {
    if (!isLiked) flyToLiked(cardRef.current)
    onLike?.(item)
  }, [item, isLiked, onLike])
  ...
  const handleDislike = useCallback(() => {
    ...
    thanosSnap(cardRef.current, () => onDislike?.(item.id))
  }, [item.id, onDislike, disintegrating])
          ...
          <Space size={8}>
            ...
            <Button
              ...
              style={{
                ...
                fontWeight: 600,
                transform: skipHovered && !disintegrating ? 'scale(1.08)' : 'scale(1)',
                transition: 'all 0.18s ease',
              }}
            >
              Skip
